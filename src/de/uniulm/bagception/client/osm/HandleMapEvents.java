...
public class HandleMapEvents extends MapView {
	...
	public HandleMapEvents(Context context, int tileSizePixels) {
		super(context, tileSizePixels);

	}

	public HandleMapEvents(Context c, AttributeSet a) {
		super(c, a);
	}
	...
	private void tap(int x, int y) {
		IGeoPoint p = HandleMapEvents.this.getProjection().fromPixels((int) x,
				(int) y);
	...
	private class RadiusItemizedOverlay extends
		...
		protected void draw(Canvas arg0, MapView arg1, boolean arg2) {
			if (itemList.size() > 0 && gpsMarkerLocation != null) {
				float projectionMeter = getProjection().metersToEquatorPixels(
						radius);
